...
export default function App() {
  ...
  // selection states
  const [selectedHobby, setSelectedHobby] = useState<Hobby | null>(null); // used for community details view
  const [selectedHobbyId, setSelectedHobbyId] = useState<string | null>(null); // used for post creation
  ...
  useEffect(() => {
    const initApp = async () => {
      ...
      try {
        ...
        if (session) {
          // Attempt to hydrate from profiles table (real name + stats)
          const { data: profile } = await supabase.from('profiles').select('id, name, email, avatar, stats').eq('id', session.user.id).single().catch(() => ({ data: null }));
          setCurrentUser({
            id: session.user.id,
  ...
  const getAutoIcon = (hobbyName: string, category?: string): string => {
    ...
    if (nameLower.includes("garden") || nameLower.includes("plant") || nameLower.includes("farm"))
      return "🌱";
    ...
    if (nameLower.includes("cook") || nameLower.includes("food"))
      return "🍳";
    if (nameLower.includes("art") || nameLower.includes("draw") || nameLower.includes("paint"))
      return "🎨";
    if (nameLower.includes("music") || nameLower.includes("guitar"))
      return "🎸";
    if (nameLower.includes("tech") || nameLower.includes("code") || nameLower.includes("program"))
      return "💻";
    if (nameLower.includes("book") || nameLower.includes("read"))
      return "📚";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Fetch profile from profiles table (real name)
    const { data: profile } = await supabase.from('profiles').select('*').eq('id', user.id).single().catch(() => ({ data: null }));

    setCurrentUser({
      id: user.id,
      name: profile?.name || user.user_metadata?.full_name || (user.email ? user.email.split('@')[0] : 'User'),
      email: user.email ?? "",
  ...
  const handleRegister = async () => {
    ...
    // Note: Supabase Auth signUp -> create user; we'll also create profile row
    ...
    if (error) {
      ...
      if (data?.user) {
        // Insert into profiles table with the name user provided
  ...
  const handleCreateHobby = async (nameArg: string, description: string, category: HobbyCategory) => {
    if (!supabase || !currentUser) return;
    setIsLoading(true);

    const icon = getAutoIcon(nameArg, category as string);
  ...
  const handleLeaveCommunity = async (hobbyId: string) => {
    ...
    setCurrentUser(prev => prev ? { ...prev, joinedHobbies: prev.joinedHobbies.filter(id => id !== hobbyId) } : null);
  ...
  const handleCreatePost = async (content: string, hobbyId?: string) => {
    ...
    if (!error) {
      setCurrentUser(prev => prev ? {
        ...prev,
        stats: { ...prev.stats, totalStreak: prev.stats.totalStreak + 1, points: prev.stats.points + 20 }
      } : null);
  ...
  // Status bar (kept simple — removed network icons as you requested earlier)
  const StatusBar = () => (
            ...
            <div className="px-6 pt-4">
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar mb-6 pb-2">
                {Object.values(HobbyCategory).map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat as HobbyCategory)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedCategory === cat ? 'bg-slate-900 text-white' : 'bg-white text-slate-500 border border-slate-100'}`}>
                    {cat}
                  </button>
                ))}
              </div>
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {hobbies.filter(h => currentUser?.joinedHobbies.includes(h.id)).map(h => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-xs flex items-center gap-2 whitespace-nowrap border transition-all ${selectedHobbyId === h.id ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-700 border-slate-200'}`}
                    >
                ...
                <Button
                  ...
                  onClick={(e: React.MouseEvent) => {
                    if (currentUser?.joinedHobbies.includes(selectedHobby.id)) {
                      handleLeaveCommunity(selectedHobby.id);
                    } else {
                      handleJoinCommunity(e, selectedHobby.id);
                    }
                  }}
