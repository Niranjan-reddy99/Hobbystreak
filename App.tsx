...
const Toast = ({ message, type = 'success' }: { message: string; type?: 'success' | 'error' }) => (
  <div
    className={`absolute top-12 left-1/2 transform -translate-x-1/2 px-6 py-3 rounded-full shadow-lg z-50 flex items-center gap-2 w-max ${
...
const Confetti = () => (
  <div className="absolute inset-0 pointer-events-none overflow-hidden z-50 flex justify-center">
    <div className="confetti-piece bg-red-500 left-[10%]" />
    <div className="confetti-piece bg-blue-500 left-[20%] delay-100" />
    <div className="confetti-piece bg-yellow-500 left-[30%] delay-200" />
    <div className="confetti-piece bg-green-500 left-[40%]" />
  </div>
);

const Button = ({ children, onClick, variant = 'primary', className = '', icon: Icon, disabled, isLoading }: any) => {
  const baseStyle =
    'px-4 py-3 rounded-2xl font-medium transition-all duration-200 flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50';
...
export default function App() {
  ...
  const getAutoIcon = (hobbyName: string, category?: string): string => {
    ...
    switch (category) {
      case HobbyCategory.FITNESS:
      case 'Fitness':
        return '💪';
      case HobbyCategory.CREATIVE:
      case 'Creative':
        return '✨';
      case HobbyCategory.TECH:
      case 'Tech':
        return '💻';
      case HobbyCategory.LIFESTYLE:
      case 'Lifestyle':
        return '🌟';
      default:
        return '⭐';
...
setUnreadCount((notifData || []).filter(n => !n.is_read).length);
  ...
  const fetchHobbiesAndPosts = async (currentUserId?: string | null) => {
    ...
    try {
      ...
      if (hobbiesData) {
        const formatted = hobbiesData.map((h: any) => ({
          id: h.id,
          name: h.name,
      ...
      if (postsData) {
        const formattedPosts: Post[] = postsData.map((p: any) => ({
          ...
          likes: p.likes ?? 0,
          comments:
            (commentsData?.filter((c: any) => c.post_id === p.id) || []).map((c: any) => ({
              id: c.id,
              userId: c.user_id,
              content: c.content,
      ...
      console.error('fetchHobbiesAndPosts error', e);
    }
  };
  ...
  const handleLeaveCommunity = async (hobbyId: string) => {
    ...
    try {
      if (!currentUser.joinedHobbies.includes(hobbyId)) return showToast('You are not a member');
  ...
  return (
    <div className="min-h-screen bg-neutral-900 flex items-center justify-center font-sans p-0 sm:p-8">
      {/* PHONE FRAME */}
      <div
        className="w-full max-w-[400px] h-[100dvh] sm:h-[850px] bg-slate-50 sm:rounded-[40px] shadow-2xl overflow-hidden relative flex flex-col border-0 sm:border-[8px] border-neutral-800 ring-1 ring-white/10"
      >
        {/* Toast + Confetti */}
                  ...
                  return (
                    <div key={post.id} className="bg-white p-5 rounded-3xl shadow-sm">
                      ...
                      <div className="flex gap-4 text-slate-400 border-t border-slate-50 pt-3">
                        ...
                        <div className="flex items-center gap-1 text-xs">
                          <button onClick={() => setExpandedPostId(expandedPostId === post.id ? null : post.id)} className="flex items-center gap-1">
          ...
          {view === ViewState.COMMUNITY_DETAILS && selectedHobby && (
            <div className="bg-white min-h-full">
              <div className="h-48 relative">
                ...
                <button onClick={() => setView(ViewState.EXPLORE)} className="absolute top-4 left-4 p-2 bg-white/20 backdrop-blur-md rounded-full text-white">
            ...
            <div className="px-6 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-white p-4 rounded-3xl shadow-sm text-center">
                  <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center text-orange-500 mx-auto mb-2"><Flame className="w-5 h-5" /></div>
              ...
              <div className="space-y-3">
                {hobbies.filter(h => currentUser?.joinedHobbies.includes(h.id)).map(h => (
                  <div key={h.id} className="bg-white p-4 rounded-2xl flex items-center gap-3 shadow-sm">
                    ...
                    <div><ArrowLeft className="w-4 h-4 text-slate-300" /></div>
                  </div>
                ))}
              </div>
            </div>
                ...
                {getWeekDays().map((date, index) => {
                  ...
                  return (
                    <button key={index} onClick={() => setSelectedDate(dateStr)} className={`flex flex-col items-center justify-center min-w-[50px] h-[70px] rounded-2xl transition-all ${isSelected ? 'bg-slate-900 text-white shadow-lg shadow-slate-900/30 scale-105' : 'bg-white text-slate-400 border border-slate-100'}`}>
                      ...
                      {isToday && !isSelected && <div className="w-1 h-1 bg-red-500 rounded-full mt-1" />}
                    </button>
                  );
                })}
              </div>
                  ...
                  return (
                    <div key={task.id} className="bg-white p-4 rounded-2xl shadow-sm flex items-center gap-3 group">
                      <button onClick={() => handleToggleTask(task.id)} className={`transition-colors ${task.completed ? 'text-green-500' : 'text-slate-200 hover:text-slate-300'}`}>
                        {task.completed ? <CheckCircle className="w-6 h-6 text-green-100" /> : <Circle className="w-6 h-6" />}
                      </button>
