...
export default function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
        ...
        // Try to recover session (might fail if storage blocked, but that's okay now)
        try {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
        ...
        // FIXED: DO NOT RELOAD PAGE. Just update state manually.
  ...
  const handleCreatePost = async (content: string) => {
      // Emergency Check: If state was lost, try to use the session directly
      ...
      if (!activeUserId) return showToast("Log in first (Session Lost)", "error");
    ...
    <div className="min-h-screen bg-neutral-900 flex items-center justify-center font-sans p-0 sm:p-8">
      <div className="w-full max-w-[400px] h-[100dvh] sm:h-[850px] bg-slate-50 sm:rounded-[40px] shadow-2xl overflow-hidden relative flex flex-col border-0 sm:border-[8px] border-neutral-800 ring-1 ring-white/10">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
            ...
            {/* Omitted other views for brevity - they are same as previous versions but NAV works */}
