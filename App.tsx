...
interface User {
  ...
  email: string;
  avatar: string;
  joinedHobbies: string[]; // List of UUID strings
  stats: {
    totalStreak: number;
    points: number;
  };
...
export default function App() {
  ...
  // --- FORCE REFRESH SELECTION ON POST VIEW ---
  useEffect(() => {
      // Whenever view changes to CREATE_POST, force a selection if none exists
      if (view === ViewState.CREATE_POST && currentUser?.joinedHobbies?.length) {
          // Always default to the first hobby if nothing is selected or if selection is invalid
          if (!selectedPostHobbyId || !currentUser.joinedHobbies.includes(selectedPostHobbyId)) {
              console.log("Auto-selecting hobby:", currentUser.joinedHobbies[0]);
              setSelectedPostHobbyId(currentUser.joinedHobbies[0]);
          }
      }
  }, [view, currentUser?.joinedHobbies, selectedPostHobbyId]);
  ...
  const fetchUserHobbies = async (userId: string) => {
      ...
      return joinedIds; // Return for immediate usage
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
        ...
        // Force a fresh fetch of everything on login
  ...
  const handleCreateHobby = async (name: string, description: string, category: HobbyCategory) => {
    ...
    if (!error && data) {
        // 2. Join it immediately in DB
        await supabase.from('user_hobbies').insert({ user_id: currentUser.id, hobby_id: data.id });
        
        // 3. OPTIMISTIC UPDATE: Manually update local state immediately
        const newJoinedList = [...currentUser.joinedHobbies, data.id];
        
        // Add new hobby to list
        const newHobby: Hobby = { ...data, memberCount: 1, image: 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f', icon: '🌟' };
        setHobbies(prev => [...prev, newHobby]);
        
        // Update user state
        setCurrentUser({ ...currentUser, joinedHobbies: newJoinedList });
        
        // Update selection state for posting
  ...
  const handleCreatePost = async (content: string, hobbyId: string) => {
    ...
    if (!hobbyId) {
        // Fallback: if state is empty, try to grab the first one from user object
        if (currentUser.joinedHobbies.length > 0) hobbyId = currentUser.joinedHobbies[0];
        else return showToast("Join a community first!", "error");
    }
                ...
                <div className="px-6 pt-4">
                     ...
                     <div className="space-y-3">
                         ...
                         {hobbies.filter(h => currentUser?.joinedHobbies.includes(h.id)).length === 0 && (
                             <p className="text-center text-xs text-slate-400 py-4">No joined hobbies yet.</p>
                         )}
                     </div>
                </div>
            )}
