...
export default function App() {
    ...
    const initApp = async () => {
        ...
        try {
            ...
            if (session) {
                ...
                await fetchHobbiesAndPosts(session.user.id); // Fetch posts with user context
        ...
        // Initial load of tasks happens in loadUserProfile
  ...
  const fetchHobbiesAndPosts = async (currentUserId: string | null = null) => {
      ...
      if (postsData) {
          // 1. Get unique user IDs to fetch names
          ...
          setPosts(postsData.map((p: any) => {
              ...
              const mappedComments = postComments.map((c: any) => {
                  ...
                  return {
                      ...
                      authorName: commentAuthor?.name || 'User' // Fetch real name if available
                  };
              });
              ...
              return {
                  ...
                  authorName: author?.name || 'Anonymous', // FIX: Uses DB name
            ...
            {view === ViewState.SCHEDULE && (
                <div className="px-6 pt-4 h-full flex flex-col">
                    ...
                    <div className="bg-white p-2 rounded-2xl shadow-lg border flex gap-2 mt-4 absolute bottom-24 left-6 right-6">
                        <input id="new-task" className="flex-1 pl-4 outline-none text-sm bg-transparent" placeholder="Add a new task..." 
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    const target = e.target as HTMLInputElement;
