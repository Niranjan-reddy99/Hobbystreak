...
export default function App() {
  ...
  useEffect(() => {
    const initApp = async () => {
        ...
        if (session) {
            ...
            await fetchJoinedHobbies(session.user.id); // Load joined hobbies immediately
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
        ...
        await fetchJoinedHobbies(data.session.user.id);
        setView(ViewState.FEED);
  ...
  // FIXED: handleCreateHobby now instantly updates local state
  ...
  // FIXED: handleJoinCommunity force refreshes
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
            ...
            {view === ViewState.CREATE_POST && (
                <div className="px-6 pt-12 h-full bg-white">
                    ...
                    {/* DEBUGGING PANEL - REMOVE AFTER FIX */}
                    <div className="bg-gray-100 p-2 text-xs mb-2 rounded font-mono">
                        DEBUG: Joined Communities = {myCommunities.length}
                    </div>
