...
export default function App() {
      ...
      if (postsData) {
          ...
          setPosts(postsData.map((p: any) => {
              ...
              return {
                  ...
                  authorName: author?.name || 'Community Member', // Real name or fallback
  ...
  const handleToggleTask = async (task: Task) => {
      ...
      const updatedTasks = tasks.map(t => t.id === task.id ? { ...t, completed: isCompleting } : t);
                        ...
                        {posts.map(post => {
                            ...
                            <div key={post.id} className="bg-white p-5 rounded-3xl shadow-sm">
                                ...
                                <div className="flex gap-4 text-slate-400 text-xs border-t pt-3 mt-3">
                                    ...
                                    <button onClick={() => setExpandedPostId(expandedPostId === post.id ? null : post.id)} className="flex items-center gap-1 hover:text-blue-500 transition-colors"><MessageCircle className="w-4 h-4"/> {post.comments.length}</button>
                                </div>
                                ...
                                {expandedPostId === post.id && (
                                    <div className="mt-4 pt-4 border-t border-slate-50 animate-in slide-in-from-top-2">
                                        ...
                                        <div className="flex gap-2">
                                            <input id={`comment-${post.id}`} className="flex-1 bg-slate-100 rounded px-3 py-2 text-xs outline-none" placeholder="Reply..." onKeyDown={(e) => { if (e.key === 'Enter') { const target = e.target as HTMLInputElement; handleComment(post.id, target.value); target.value = ''; }}} />
                                            <button onClick={() => { const input = document.getElementById(`comment-${post.id}`) as HTMLInputElement; if (input.value) { handleComment(post.id, input.value); input.value = ''; }}} className="p-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700"><Send className="w-3 h-3" /></button>
                                        </div>
                                    </div>
                                )}
                            </div>
                                     ...
                                     {expandedPostId === post.id && (
                                        <div className="mt-4 pt-4 border-t border-slate-50 animate-in slide-in-from-top-2">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button onClick={() => { const input = document.getElementById(`comm-comm-${post.id}`) as HTMLInputElement; if (input.value) { handleComment(post.id, input.value); input.value = ''; } }} className="p-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700"><Send className="w-3 h-3" /></button>
                                            </div>
                                        </div>
                                    )}
                                 </div>
