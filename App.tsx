...
export default function App() {
  ...
  const [joinedHobbyIds, setJoinedHobbyIds] = useState<string[]>([]); // Separated for stability
  ...
  const refreshPublicData = async () => {
      ...
      if (postsData) {
          // In real app, utilize a join. For MVP, we map simply.
  ...
  const loadUserProfile = async (userId: string) => {
      ...
      // 2. Get Joined Hobbies (CRITICAL FOR POSTING)
      ...
      if (joined) {
          ...
          setJoinedHobbyIds(ids); // Update the separated state
          if (ids.length > 0) setSelectedPostHobbyId(ids[0]); // Auto-select first for posting
      }
  };
  ...
  const handleCreateHobby = async (name: string, desc: string, cat: HobbyCategory) => {
      if (!currentUser || !supabase) return;
      setIsLoading(true);

      // 1. Insert Hobby
      const { data, error } = await supabase.from('hobbies').insert({
          name, description: desc, category: cat, icon: '🌟', member_count: 1
      }).select().single();
      ...
      if (data && !error) {
          ...
          // 3. Update Local State INSTANTLY (Fixes glitch)
  ...
  const handleJoinCommunity = async (e: React.MouseEvent, hobbyId: string) => {
      ...
      if (joinedHobbyIds.includes(hobbyId)) {
      ...
      if (!error) {
          // Optimistic Update (Fixes glitch)
  ...
  const handleCreatePost = async (content: string) => {
      if (!currentUser || !supabase) return;
      if (!selectedPostHobbyId) return showToast("Select a community first!", "error");
  ...
  const handleAddTask = (title: string) => {
      const newTask: Task = { id: `t${Date.now()}`, title, date: selectedDate, completed: false };
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
            ...
            {view === ViewState.REGISTER && (
                <div className="h-full flex flex-col justify-center px-8">
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={() => setView(ViewState.LOGIN)} className="block w-full text-center mt-4 text-sm text-slate-400">Back to Login</button>
                    </div>
                </div>
            )}
                ...
                <div className="px-6 pt-4">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
                        {Object.values(HobbyCategory).map(cat => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap ${selectedCategory === cat ? 'bg-slate-900 text-white' : 'bg-white border'}`}>
                                {cat}
                            </button>
                        ))}
                    </div>
                            ...
                            .map(h => {
                                ...
                                return (
                                    <div key={h.id} className="bg-white p-4 rounded-3xl flex items-center gap-4 shadow-sm" onClick={() => { setSelectedHobby(h); setView(ViewState.COMMUNITY_DETAILS); }}>
                                        ...
                                        <button 
                                            onClick={(e) => handleJoinCommunity(e, h.id)} 
                                            className={`px-4 py-2 rounded-xl text-xs font-bold ${isJoined ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-900'}`}
                                        >
                ...
                <div className="px-6 pt-12 h-full bg-white">
                    ...
                    <div className="mt-4">
                        <div className="flex justify-between mb-2">
                            ...
                            <button onClick={() => loadUserProfile(currentUser!.id)} className="text-xs text-blue-500 flex items-center gap-1"><RefreshCw className="w-3 h-3"/> Refresh</button>
                        </div>
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-4">
                            {myCommunities.map(h => (
                                <button 
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs border flex items-center gap-2 whitespace-nowrap ${selectedPostHobbyId === h.id ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600'}`}
                                >
                    ...
                    <Button className="w-full mt-8" onClick={() => {
                        ...
                        if(content) handleCreatePost(content);
                    }}>Post Update</Button>
                </div>
            )}
            ...
            {view === ViewState.CREATE_HOBBY && (
                <div className="px-6 pt-12 h-full bg-white">
                    ...
                    <div className="space-y-4">
                        <input id="new-hobby-name" className="w-full p-4 bg-slate-50 rounded-2xl" placeholder="Name" />
                        <textarea id="new-hobby-desc" className="w-full p-4 h-24 bg-slate-50 rounded-2xl" placeholder="Description" />
                        <select id="new-hobby-cat" className="w-full p-4 bg-slate-50 rounded-2xl">
                            {Object.values(HobbyCategory).filter(c => c !== 'All').map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                        <Button className="w-full mt-4" onClick={() => {
                            const name = (document.getElementById('new-hobby-name') as HTMLInputElement).value;
                            const desc = (document.getElementById('new-hobby-desc') as HTMLTextAreaElement).value;
                            const cat = (document.getElementById('new-hobby-cat') as HTMLSelectElement).value as HobbyCategory;
                ...
                <div className="px-6 pt-4 h-full flex flex-col">
                    ...
                    <div className="flex justify-between mb-6">
                        {getWeekDays().map((d, i) => {
                            ...
                            return (
                                <button key={i} onClick={() => setSelectedDate(d.toISOString().split('T')[0])} className={`flex flex-col items-center justify-center w-10 h-14 rounded-xl text-xs ${isSelected ? 'bg-slate-900 text-white' : 'bg-white border'}`}>
                                    <span className="uppercase font-bold">{['S','M','T','W','T','F','S'][d.getDay()]}</span>
                                    <span>{d.getDate()}</span>
                                </button>
                            )
                        })}
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2">
                        {tasks.filter(t => t.date === selectedDate).map(t => (
                            <div key={t.id} className="bg-white p-4 rounded-xl flex items-center gap-3 shadow-sm">
                                ...
                                <button onClick={() => { const updated = tasks.filter(x => x.id !== t.id); setTasks(updated); localStorage.setItem('hobbystreak_tasks', JSON.stringify(updated)); }} className="ml-auto text-slate-300"><Trash2 className="w-4 h-4"/></button>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="bg-white p-2 rounded-2xl shadow-lg border flex gap-2 mt-4 absolute bottom-24 left-6 right-6">
                        ...
                        <button onClick={() => {
                            const val = (document.getElementById('new-task') as HTMLInputElement).value;
            ...
            {view === ViewState.PROFILE && (
                <div className="px-6 pt-4">
                    ...
                    <div className="text-center mb-8">
                        <img src={currentUser?.avatar} className="w-24 h-24 rounded-full mx-auto mb-4 bg-slate-200" />
                        <h2 className="text-xl font-bold">{currentUser?.name}</h2>
                    </div>
                    <h3 className="font-bold text-sm mb-4">Joined Communities</h3>
                    <div className="space-y-3">
                        {myCommunities.map(h => (
                            <div key={h.id} className="bg-white p-4 rounded-2xl flex gap-3 shadow-sm">
                                <span className="text-2xl">{h.icon}</span>
                                <span className="font-bold text-sm my-auto">{h.name}</span>
                            </div>
                        ))}
                        {myCommunities.length === 0 && <p className="text-slate-400 text-sm text-center">No communities yet.</p>}
                    </div>
                </div>
            )}
